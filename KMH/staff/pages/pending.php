...
$cr = "SELECT * FROM `tbl_reserved` WHERE pendingAmount != '0' AND roomStatus = 'active'";
$tl = mysql_query($cr);
...
<div class="container-fluid">
    <div class="row">
        ...
        <?php
        $sql = "SELECT * FROM `tbl_reserved`  WHERE pendingAmount != '0' AND roomStatus = 'active' LIMIT $page,7;";
        ...
        <td><?php echo ($row['amountPaid'])*3?></td>

        <td>
        <a href="checkIn.php?id=<?=$row["recieptCode"];?>">
        <i class="fa fa-sign-in fa-fw" style="font-size:20px;" data-toggle="tooltip" data-placement="left" title="Check In"></i></a>
        </td>
        </tr>
        <?php
        }
        ...
        if ($check<1) {
            echo "There are no pending rooms at the monent";
        }
        ?>
        </td>
        </tr>
        ...
        <?php
        $cou = "SELECT * FROM `tbl_reserved` WHERE pendingAmount != '0' AND roomStatus = 'active'";
        ...
        for ($b=1; $b <=$a ; $b++) { 
            ?>    
         <span class='label label-info'><a href = "pending.php?page=<?php echo $b;?>"style = "text-decoration:none;"><?php echo $b," "; ?></a></span>
        <?php
        }
        ?>        
        </td>
